...
void main() {
  ...
  group('JUNIOR-3 runtime factory', () {
    ...
    test(
      'inexperienced candidates cannot enter pre-join sales and join via salary SSOT',
      () {
        final candidate = applicant(
          id: 'junior',
          experienceMonths: 0,
    ...
    test(
      'junior profiles are deterministic and both numeric templates are reachable',
      () {
        final potential = applicant(
          id: 'a',
          experienceMonths: 0,
          interviewScore: 64,
          acceptanceScore: 76,
        ...
        expect(
          PublicDemoEngineerRuntime.fromApplicant(potential).toJson(),
          a.toJson(),
        );
      },
    );
    ...
    test(
      'junior starts with no work or industry history and supports waiting/training growth',
      () {
        ...
        final waiting = PublicDemoGrowthEngine.calculate(
          runtime,
        ...
        final training = PublicDemoGrowthEngine.calculate(
          runtime,
          const PublicDemoGrowthRequest(
        ...
        expect(
          ...
          reason:
              'the established deterministic waiting formula may round to 0',
        );
        expect(
          training.after.actualCapability,
          greaterThan(runtime.actualCapability),
        );
        expect(
          PublicDemoState.aprilStart()
              .selectInternalTraining(runtime.engineerId)
    ...
    test(
      'assignment remains the only source that adds one experience month',
      () {
        final runtime = PublicDemoEngineerRuntime.fromApplicant(
          applicant(
            id: 'junior',
        ...
        final assignment = PublicDemoGrowthEngine.calculate(
          runtime,
          const PublicDemoGrowthRequest(
    ...
    test(
      'field sales readiness is a pure 60-point boundary and JSON stays compatible',
      () {
        final below = PublicDemoEngineerRuntime.fromApplicant(
          applicant(
            id: 'junior',
            experienceMonths: 0,
            interviewScore: 64,
        ...
        expect(
          PublicDemoEngineerRuntime.fromJson(at.toJson()).toJson(),
          at.toJson(),
        );
      },
    );
