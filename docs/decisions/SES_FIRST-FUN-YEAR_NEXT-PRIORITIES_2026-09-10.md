...
4. ~~**Employee lifecycle status clarity**~~ — **Done (2026-09-11).** Make waiting → sales-ready/in-sales → interview → ordered → next-month assignment → assigned understandable and consistent across surfaces. Implemented as `PublicDemoEmployeeStatusResolver` (a single pure resolver shared by the 社員タブ roster and both SkillSheet call sites); fixed the two inconsistencies the Fresh Audit found (SkillSheet stale 翌月参画予定 for an already-assigned engineer; training-selection overriding an otherwise-correct label's tone) plus one related label/tone gap found during implementation. HOME's own status function is intentionally left unrouted through the shared resolver this round (HOME Freeze — see the governing plan's 2026-09-11 Update history entry and `docs/reports/SES_FIRST-FUN-YEAR_Employee-Status-Unified-Display_Result.md` for the full rationale and test evidence).
