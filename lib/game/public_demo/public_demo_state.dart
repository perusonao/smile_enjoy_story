...
class PublicDemoState {
  ...
  /// Starts a brand-new playthrough (SEEDED-RNG-REUSE-1). [runSeed] is the
  /// stable per-playthrough seed every future independently-reproducible
  /// content stream (recruitment candidates, interviews, project
  /// generation -- see [PublicDemoRngNamespace]) will eventually derive
  /// from via `PublicDemoRng`; when the caller doesn't supply one, a fresh
  /// value is drawn the same way [GameEngine.newGame]/[PrologueEngine
  /// .newGame] already draw the main game's own market seed --
  /// `DateTime.now().millisecondsSinceEpoch & 0x7fffffff` -- so "4月から
  /// もう一度" (a fresh call to this factory) always starts a new,
  /// independent playthrough's seed, never the abandoned one's.
  factory PublicDemoState.aprilStart({int? runSeed}) => PublicDemoState(
    month: 4,
    cash: 4000000,
    engineerCount: 2,
    adminCount: 1,
    ...
    engineersWaiting: 2,
    engineersAssigned: 0,
    runSeed: runSeed ?? (DateTime.now().millisecondsSinceEpoch & 0x7fffffff),
  );
  final int month,
      cash,
      engineerCount,
  ...
  /// The stable seed for this one playthrough (SEEDED-RNG-REUSE-1),
  /// generated once by [aprilStart] and carried unchanged through every
  /// later [copyWith]/month transition/save-reload -- never regenerated by
  /// any of them. Future independently-reproducible content streams
  /// (recruitment candidates, interviews, project generation) derive their
  /// own per-stream seed from this value via `PublicDemoRng`
  /// (`public_demo_rng.dart`), never by constructing `Random()` directly.
  final int runSeed;
  ...
  PublicDemoState copyWith({
    ...
    latestMonthlyCashFlow: latestMonthlyCashFlow,
    // runSeed has no parameter here deliberately: it is fixed for the whole
    // playthrough at [aprilStart] and every ordinary command goes through
    // this public copyWith, never [_copyWith] directly -- [_copyWith]'s own
    // `runSeed` parameter exists only for [fromJson] restoring a persisted
    // value, which always forwards `this.runSeed` below unchanged anyway.
  );
  ...
  PublicDemoState _copyWith({
    ...
    // Always this.runSeed, never a parameter: every ordinary transition
    // reaches [PublicDemoState] construction only through here, so this is
    // what actually guarantees runSeed survives unchanged across every
    // month/command -- see [runSeed]'s own doc.
    runSeed: runSeed,
  );
  ...
  factory PublicDemoState.fromJson(
    ...
    latestMonthlyCashFlow: json['latestMonthlyCashFlow'] is Map
        ...
        : null,
    runSeed: json['runSeed'] is int
        ? json['runSeed'] as int
        : _legacyRunSeedFallback(json),
  );

  /// SEEDED-RNG-REUSE-1: a save persisted before [runSeed] existed carries
  /// no `runSeed` key (or, if hand-corrupted, a non-int one) at all. Rather
  /// than reject the whole save the way a genuinely inconsistent field
  /// would be, this derives a stable replacement from the save's own other
  /// fields via [stableHash] -- the same primitive `lib/game/engine/rng.dart`
  /// already uses for every other derived seed, never a new hash -- so the
  /// exact same legacy payload always resolves to the exact same runSeed on
  /// every future reload, even before this migrated value is first written
  /// back by a save. [PublicDemoSaveCodec] relies on this being a pure
  /// function of [json] to accept such a save without otherwise weakening
  /// its own strict round-trip check for every other field.
  static int _legacyRunSeedFallback(Map<String, dynamic> json) =>
      stableHash(jsonEncode(json));
