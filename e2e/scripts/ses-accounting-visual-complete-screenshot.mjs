// SES NON-HOME-UI ACCOUNTING Visual Complete: Screen Verification capture —
// serves build/web on a local port and screenshots the 会計 tab at the two
// required mobile viewports. Throwaway script, mirrors
// e2e/scripts/ses-sales-visual-complete-screenshot.mjs's own server/browser
// setup.
...
const PORT = Number(process.argv[5] || 8938);
...
for (const size of sizes) {
  ...
  const nav = page.getByRole('tab', { name: '会計', exact: true }).or(
    page.getByRole('button', { name: '会計', exact: true }),
  );
  await nav.first().click();
  await page.waitForTimeout(800);

  const outPath = `${OUT_DIR}/SES_NON-HOME-UI_ACCOUNTING_Visual-Complete_${LABEL}_${size.name}.png`;
