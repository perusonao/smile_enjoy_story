...
void main() {
  group('PUBLIC-DEMO-UX-1A SkillSheet inspection', () {
    testWidgets('HOME action opens truthful content and back does not advance', (
      tester,
    ) async {
      ...
      expect(find.text('${engineer.interviewProfile.skillFit}'), findsWidgets);
      expect(find.text('ヒューマンスキル'), findsOneWidget);
      expect(find.text('${engineer.interviewProfile.humanity}'), findsWidgets);
      ...
      expect(find.text('${engineer.interviewProfile.clientTrust}'), findsWidgets);
    ...
    testWidgets('explicit confirmation advances once and existing sales start continues', (
      ...
      // HOME route share the same inspect-before-advance behavior.
      final skillSheetButton = find.widgetWithText(FilledButton, 'SkillSheet確認');
