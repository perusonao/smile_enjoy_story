...
Before the Codex Broad Review round:

- `flutter analyze` (whole project): **No issues found.**
- `flutter test test/game/public_demo`: **944/944 passed** (51 new Phase 1a domain tests + 4 new save-codec tests).
- `flutter test test/ui/public_demo`: **741/741 passed** — zero UI regression, as expected for a domain-only change.
- Full-repository `flutter test` (every suite, not just the two above) was also run to completion in the background and finished **2482/2482 passed** before the review round began — confirming the whole-repo, not just the two required suites, was already green.

After implementing/reverting the four Codex Broad Review fixes above (see that section for detail — one candidate fix was tried, found to conflict with `PublicDemoSaveCodec`'s round-trip invariant, and reverted in favor of documentation):

- `flutter analyze` (whole project): **No issues found.**
- `flutter test test/game/public_demo`: **952/952 passed** (adds 6 new aggregate-safe-entry-point tests and 3 new save-codec score-plausibility/duplicate tests on top of the earlier 944).
- `flutter test test/ui/public_demo`: re-run to confirm the review-round fixes (all under `lib/game/`) caused no UI regression.
- `git diff --check`: clean, no whitespace errors, across the entire branch.
...
## Codex Broad Review (automated, PR #254) — findings and resolutions

This repository runs an automated Codex review on PR open (`chatgpt-codex-connector[bot]`), which fired once on the initial commit, exactly satisfying this task's own "実装完了後にCodex Broad Reviewを1回だけ行う" requirement without a separate manual step. Four findings came back (three P1, one P2); all four were investigated and each was either fixed or explicitly, honestly declined with a documented reason — none were silently ignored.

1. **P1, FIXED-WITH-CAVEAT — legacy-migration staleness.** The finding: since `PublicDemoWorkflowState.toJson()` always writes a present `offerCandidates` key (even `[]`), the very first save this Phase 1a's own code writes permanently disables the "key absent" migration trigger for that save, even though the legacy engineer pipeline keeps genuinely advancing afterward (Phase 1a never cuts that pipeline over). **An attempted code fix (re-deriving the legacy projection on every load instead of once) was implemented, tested, found to break `PublicDemoSaveCodec`'s own strict round-trip check for every save with an engineer at a relevant legacy stage (20+ pre-existing tests failed), and reverted.** Root cause: that check treats any difference between a freshly re-decoded aggregate and its own just-encoded envelope as corruption — re-deriving non-empty content from legacy facts on every decode necessarily produces exactly that difference for any real Phase 1a save (which always encodes `[]`, since nothing wires the new command path into gameplay yet). This is a genuine architectural conflict, not a narrow bug: closing it properly requires a real, one-time reconciliation Phase 1b performs itself at cutover (when it starts actually trusting/writing this list), not a per-load re-derivation Phase 1a's own additive-only mandate has any safe way to perform. The limitation is now explicitly documented at `_synthesizeLegacyOfferCandidates`'s own doc comment (`public_demo_workflow_state.dart`) so Phase 1b does not inherit a false "key present ⇒ fully migrated" assumption. No player data is ever lost — the engineer's own legacy `stage`/`interviewRecord` remain complete and correct regardless; only the *mirror* into the new list can lag.
2. **P1, FIXED — candidate interviews were a caller-controlled, unsafe-to-wire building block.** The finding: `PublicDemoWorkflowState.evaluatePartnerInterviewForCandidate`/`evaluateClientInterviewForCandidate` accept `profile`/`actualCapability` as plain parameters and never touch `salesUsed`, unlike the engineer-level route (which derives both from authoritative state and consumes a real sales slot at the `PublicDemoAggregate` layer) — a real risk if some future Phase 1b wiring reached for the workflow-level method directly instead of a safe aggregate wrapper, since none existed. Added `PublicDemoAggregate.evaluatePartnerInterviewForCandidate`/`evaluateClientInterviewForCandidate`, mirroring `recordEngineerInterviewResult` exactly: profile/capability always derived from the engineer's own `interviewProfile`/runtime, a real sales slot checked-then-consumed for the partner attempt, none for the 0-slot client stage. The workflow-level methods' own doc comments now explicitly cross-reference these as "the safe production entry point," matching how `recordEngineerInterviewResult`'s own doc already frames that same split. Verified with 6 new tests (slot consumption, slot exhaustion never partially consuming, no-op with no candidate, derivation from real state).
3. **P1, ALREADY RESOLVED — governing plan doc.** The finding (raised against the first commit, `550035f`) was that `docs/decisions/SES_DEVELOPMENT-PRIORITY_2026-09-02.md` still framed Phase 1a as upcoming work. This was already fixed in the very next commit on this branch (`cd648e0`, pushed before this review comment was even read), which records Phase 1a's completion as the newest `## Update history` entry. Confirmed still correct; no further action needed.
4. **P2, FIXED — unbacked offer-candidate interview records were not rejected on restore.** The finding: `PublicDemoSaveCodec._hasConsistentAuthorityFacts` didn't inspect `offerCandidates` at all, so a hand-edited save could assert `interviewRecordEngineerId`/`interviewRecordProjectId` matching a candidate's own identity (passing `PublicDemoAggregate._validateForPersistence`'s identity-only check) with a fabricated `clientScore` — e.g. `10`, which `PublicDemoInterviewEvaluator` can never actually mint a genuine pass at (its hard floor is `60`). Added the composite-key generalization of this same method's existing engineer-record score-plausibility check: for any offer-candidate entry claiming a record, its identity must match its own `(engineerId, projectId)` **and** `clientScore >= 60`, exactly mirroring where the analogous engineer-level check already lives (this raw-JSON pre-check, not `_validateForPersistence` — the codebase's own established split between "identity" and "genuineness" checks). Verified with 3 new tests (implausible score rejected, duplicate raw entry rejected at this same pre-check layer, and a genuine end-to-end pass still round-trips normally).

All four resolutions were validated against the full required test matrix again after the fixes: `flutter analyze` clean, `flutter test test/game/public_demo` 952/952 passed, `flutter test test/ui/public_demo` unaffected (re-run to confirm — see Tests/results).
...
- **Legacy-migration staleness (Codex Broad Review finding #1, see that section above for the full analysis and why it was not code-fixed).** The migration synthesizes from legacy authority only once per save, gated on the `offerCandidates` key being entirely absent — never again after this Phase 1a's own `toJson` first writes that key (even as `[]`). An engineer who reaches a relevant legacy stage *after* a player's first post-Phase-1a save will not get a mirrored candidate merely by being loaded again; their real progress stays completely intact on `PublicDemoEngineerSales.stage`/`interviewRecord`, just not yet reflected into `offerCandidates`. Phase 1b must perform its own one-time reconciliation pass across existing saves at cutover time — it must NOT assume "an `offerCandidates` entry exists for every relevant-stage engineer" going in.
- The full, whole-repository `flutter test` run (beyond the two suites this task requires) completed successfully in the background before the Codex Broad Review round (2482/2482) — see Tests/results above.
...
5. Perform a genuine **one-time reconciliation pass** across existing saves at cutover time, rather than trusting "an `offerCandidates` entry already exists" — see Known limitations' "Legacy-migration staleness" entry above for exactly why this Phase 1a's own per-load, key-presence-gated synthesis cannot be relied on to have caught every relevant-stage engineer by the time Phase 1b ships.
6. Wire real UI/gameplay callers through `PublicDemoAggregate.evaluatePartnerInterviewForCandidate`/`evaluateClientInterviewForCandidate` (added during this session's Codex review response) — never directly through the `PublicDemoWorkflowState`-level methods of the same name, which accept `profile`/`actualCapability` as plain parameters and consume no sales slot by design (building blocks only, mirroring `recordEngineerInterviewResult`'s own split).
