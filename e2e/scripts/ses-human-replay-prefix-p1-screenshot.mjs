// SES HUMAN-REPLAY PRE-FIX P1 (Employee + Accounting): Screen Verification
// capture — serves build/web on a local port and screenshots both the 社員
// and 会計 tabs at the two required mobile viewports. Throwaway script,
// mirrors e2e/scripts/ses-employee-visual-complete-screenshot.mjs's own
...
const PORT = Number(process.argv[5] || 8937);
...
for (const size of sizes) {
  ...
  for (const tab of tabs) {
    const nav = page.getByRole('tab', { name: tab.label, exact: true }).or(
      page.getByRole('button', { name: tab.label, exact: true }),
    );
    await nav.first().click();
    await page.waitForTimeout(800);

    const outPath = `${OUT_DIR}/SES_HUMAN-REPLAY_PreFix_${tab.key}_${LABEL}_${size.name}.png`;
