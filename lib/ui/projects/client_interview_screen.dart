...
class ClientInterviewScreen extends StatelessWidget{
  const ClientInterviewScreen({super.key,required this.applicationId});final String applicationId;
  @override Widget build(BuildContext context){final state=context.game.state;final p=state.proposals.where((p)=>p.id==applicationId).firstOrNull;if(p==null)return const Scaffold(body:Center(child:Text('案件が見つかりません')));final e=state.engineerById(p.engineerId);final sessions=state.clientInterviews.where((s)=>s.applicationId==applicationId).toList();if(sessions.isEmpty){WidgetsBinding.instance.addPostFrameCallback((_){context.game.startClientInterview(applicationId);});return const Scaffold(body:Center(child:CircularProgressIndicator()));}final s=sessions.last;if(s.completed){
    if(!state.foundingProgress.hasSeen(OneTimeEvent.clientInterviewCelebration)){
