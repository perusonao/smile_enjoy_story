...
class PublicDemoState {
  ...
  PublicDemoState advanceToMay({required int monthlyExpenses,required int orderedEngineers}){
    if(month!=4)return this; final assigned=orderedEngineers.clamp(0,engineerCount);
    return copyWith(month:5,cash:cash-monthlyExpenses,salesUsed:0,engineersAssigned:assigned,engineersWaiting:engineerCount-assigned);
  }

  PublicDemoState advanceToJune({required int monthlyExpenses,required int acceptedHires,required int hiredWithOrders}){
    if(month!=5)return this;
    final hires=acceptedHires<0?0:acceptedHires;
    final ordered=hiredWithOrders.clamp(0,hires);
    return copyWith(month:6,cash:cash-monthlyExpenses,salesUsed:0,engineerCount:engineerCount+hires,engineersAssigned:engineersAssigned+ordered,engineersWaiting:engineersWaiting+(hires-ordered));
  }
