...
class _PublicDemo01PlaceholderScreenState extends State<PublicDemo01PlaceholderScreen> {
  ...
  void _setEngineerStage(int i, PublicDemoSalesStage stage) => setState(() {
        final next = [..._engineers]; next[i] = next[i].copyWith(stage: stage); _engineers = next;
      });
  void _setApplicantStage(int i, PublicDemoApplicantStage stage) => setState(() {
        final next = [..._applicants]; next[i] = next[i].copyWith(stage: stage); _applicants = next;
      });
  ...
  void _runEngineerInterview(int i, PublicDemoInterviewType type) {
    ...
    final r = PublicDemoInterviewEvaluator.evaluate(type: type, profile: e.interviewProfile);
    setState(() {
      if (type == PublicDemoInterviewType.partner) _state = _state.useSalesSlot();
      final next = [..._engineers];
      next[i] = e.copyWith(stage: switch ((type, r.passed)) {
  ...
  void _interviewApplicant(int i) {
    if (_state.salesRemaining <= 0) return;
    setState(() { _state = _state.useSalesSlot(); });
    _setApplicantStage(i, PublicDemoApplicantStage.interviewed);
  }

  void _offerApplicant(int i) {
    final a = _applicants[i];
    _setApplicantStage(i, a.acceptanceScore >= 60 ? PublicDemoApplicantStage.offerAccepted : PublicDemoApplicantStage.offerDeclined);
  }
  ...
  void _preEntryPartnerInterview(int i) {
    ...
    setState(() {
      ...
      next[i] = a.copyWith(stage: a.salesSkillFit >= 60 ? PublicDemoApplicantStage.preEntryPartnerPassed : PublicDemoApplicantStage.preEntryPartnerFailed);
      _applicants = next;
    });
  }

  Widget _engineerCard(int i) {
    final e = _engineers[i];
  ...
  Widget _applicantCard(int i) {
    ...
    return Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (a.stage == PublicDemoApplicantStage.interviewed) ...[
        ...
        Wrap(spacing: 8, children: [FilledButton(onPressed: a.interviewScore >= 60 ? () => _offerApplicant(i) : null, child: const Text('合格・内定')), OutlinedButton(onPressed: () => _setApplicantStage(i, PublicDemoApplicantStage.rejected), child: const Text('不採用'))]),
      ],
      if (a.stage == PublicDemoApplicantStage.offerAccepted) ...[
        const Text('内定承諾：6月入社予定'),
        FilledButton.tonal(onPressed: () => _setApplicantStage(i, PublicDemoApplicantStage.preEntrySkillSheet), child: const Text('入社前SkillSheetを確認')),
      ],
      ...
      if (a.stage == PublicDemoApplicantStage.preEntryPartnerPassed) FilledButton(onPressed: () => _setApplicantStage(i, PublicDemoApplicantStage.juneOrdered), child: const Text('6月分の発注を受注')),
      ...
      if (a.stage == PublicDemoApplicantStage.juneOrdered) const Text('6月入社と同時に案件参画予定'),
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: ListView(padding: const EdgeInsets.all(16), children: [
      ...
      Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('現預金 ${_yen(_state.cash)}'), Text('在籍 技術者${_state.engineerCount}名 / 総務${_state.adminCount}名'), Text('参画 ${_state.engineersAssigned}名 / 待機 ${_state.engineersWaiting}名'), Text('営業対応 ${_state.salesUsed}/${_state.salesCapacity}（残り${_state.salesRemaining}枠）'),
      ]))),
      if (_state.month == 4) ...[
        Text('技術者', style: Theme.of(context).textTheme.titleLarge), for (var i = 0; i < _engineers.length; i++) _engineerCard(i), OutlinedButton(onPressed: _finishApril, child: const Text('4月を終了して5月へ')),
      ] else ...[
        Text('5月：応募者一覧', style: Theme.of(context).textTheme.titleLarge), const Text('採用面談と上位会社面談は同じ月4営業枠を使用します。'), for (var i = 0; i < _applicants.length; i++) _applicantCard(i),
        const SizedBox(height: 12), Text('社員状況', style: Theme.of(context).textTheme.titleLarge), for (final e in _engineers) ListTile(title: Text(e.name), subtitle: Text(e.stage == PublicDemoSalesStage.ordered ? '5月：案件参画' : '5月：待機')),
      ],
    ])),
  );
}
