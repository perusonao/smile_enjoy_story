// SES NON-HOME-UI SALES Visual Complete: Screen Verification capture —
// serves build/web on a local port and screenshots the 営業 tab at the two
// required mobile viewports. Throwaway script, mirrors
// e2e/scripts/ses-employee-visual-complete-screenshot.mjs's own
...
for (const size of sizes) {
  ...
  const nav = page.getByRole('tab', { name: '営業', exact: true }).or(
    page.getByRole('button', { name: '営業', exact: true }),
  );
  await nav.first().click();
  await page.waitForTimeout(800);

  const outPath = `${OUT_DIR}/SES_NON-HOME-UI_SALES_Visual-Complete_${LABEL}_${size.name}.png`;
