...
void main() {
  final joined = const PublicDemoApplicant(
    id: 'raise-hire', name: 'Raise Hire', resumeSummary: 'Java 3年',
    interviewScore: 70, acceptanceScore: 70, salesSkillFit: 70,
    acceptedMonthlySalary: 320000,
  ).join(week: 9);

  test('a joined employee can request one raise from June and sees all three choices', () {
  ...
  test('raise affects salary only from the following month', () {
    ...
    expect(PublicDemoSalaryFinance.monthlyExpenses(
      baselineExpenses: 800000, hires: [decided], month: 6,
    ), 800000);
    expect(PublicDemoSalaryFinance.monthlyExpenses(
      baselineExpenses: 800000, hires: [decided], month: 7,
    ), 860000);
  });

  test('decision applies morale, trust, and history exactly once', () {
    final once = joined.decideRaise(
      decisionMonth: 6, week: 24, decision: PublicDemoRaiseDecision.hold,
    );
    ...
    expect(once.relationshipHistory, hasLength(joined.relationshipHistory.length + 1));
    expect(once.relationshipHistory.last.reason, '昇給希望を今回は据え置き');
    expect(twice.raisedMonthlySalary, once.raisedMonthlySalary);
    expect(twice.relationshipHistory, hasLength(once.relationshipHistory.length));
  ...
  test('recovery path without a hire still has no raise request', () {
    const applicant = PublicDemoApplicant(
      id: 'unjoined', name: 'Unjoined', resumeSummary: 'Java 2年',
      interviewScore: 70, acceptanceScore: 70, salesSkillFit: 70,
    );
