...
class ClientInterviewSession {
  ...
  /// Which [SelectionStep] this session resolves (Playable 0.5A §45-49):
  /// [SelectionStep.clientInterview] for every pre-0.5A session (the only
  /// kind that used to exist), or [SelectionStep.upperCompanyInterview] for
  /// the Founding Prologue's shorter, player-attended upper-company round.
  /// Purely a tag read by [GameEngine]/[ClientInterviewEngine] to know which
  /// step to resolve against — the question/evaluation machinery itself is
  /// identical either way.
  final SelectionStep step;

  const ClientInterviewSession({required this.id,required this.applicationId,required this.employeeId,required this.projectId,required this.clientId,required this.startedWeek,this.currentQuestionIndex=0,required this.questions,this.employeeAnswers=const[],this.playerFollowUps=const[],this.interviewerReactions=const[],this.accumulatedEvaluation=const ClientInterviewEvaluation(),this.completed=false,this.deepDiveOccurred=false,this.mismatchFailure=false,this.deepDiveText,this.result,this.step=SelectionStep.clientInterview});
  // [questions] is settable only for the legacy-save display sanitization
  // pass (see ClientInterviewEngine.sanitized/sanitizeLegacySessions) — every
  // gameplay call site below still passes none, leaving the question list
  // (category/mismatch — the authoritative fields) exactly as generated.
  ClientInterviewSession copyWith({int? currentQuestionIndex,List<ClientInterviewQuestion>? questions,List<ClientInterviewAnswer>? employeeAnswers,List<ClientInterviewFollowUp>? playerFollowUps,List<String>? interviewerReactions,ClientInterviewEvaluation? accumulatedEvaluation,bool? completed,bool? deepDiveOccurred,bool? mismatchFailure,String? deepDiveText,ClientInterviewResult? result})=>ClientInterviewSession(id:id,applicationId:applicationId,employeeId:employeeId,projectId:projectId,clientId:clientId,startedWeek:startedWeek,currentQuestionIndex:currentQuestionIndex??this.currentQuestionIndex,questions:questions??this.questions,employeeAnswers:employeeAnswers??this.employeeAnswers,playerFollowUps:playerFollowUps??this.playerFollowUps,interviewerReactions:interviewerReactions??this.interviewerReactions,accumulatedEvaluation:accumulatedEvaluation??this.accumulatedEvaluation,completed:completed??this.completed,deepDiveOccurred:deepDiveOccurred??this.deepDiveOccurred,mismatchFailure:mismatchFailure??this.mismatchFailure,deepDiveText:deepDiveText??this.deepDiveText,result:result??this.result,step:step);
  Map<String,dynamic> toJson()=>{'id':id,'applicationId':applicationId,'employeeId':employeeId,'projectId':projectId,'clientId':clientId,'startedWeek':startedWeek,'currentQuestionIndex':currentQuestionIndex,'questions':questions.map((e)=>e.toJson()).toList(),'employeeAnswers':employeeAnswers.map((e)=>e.toJson()).toList(),'playerFollowUps':playerFollowUps.map((e)=>e.name).toList(),'interviewerReactions':interviewerReactions,'accumulatedEvaluation':accumulatedEvaluation.toJson(),'completed':completed,'deepDiveOccurred':deepDiveOccurred,'mismatchFailure':mismatchFailure,'deepDiveText':deepDiveText,'result':result?.name,'step':step.jsonValue};
  factory ClientInterviewSession.fromJson(Map<String,dynamic> j)=>ClientInterviewSession(id:j['id'] as String,applicationId:j['applicationId'] as String,employeeId:j['employeeId'] as String,projectId:j['projectId'] as String,clientId:j['clientId'] as String,startedWeek:j['startedWeek'] as int,currentQuestionIndex:j['currentQuestionIndex'] as int,questions:(j['questions'] as List).map((e)=>ClientInterviewQuestion.fromJson(e as Map<String,dynamic>)).toList(),employeeAnswers:(j['employeeAnswers'] as List).map((e)=>ClientInterviewAnswer.fromJson(e as Map<String,dynamic>)).toList(),playerFollowUps:(j['playerFollowUps'] as List).map((e)=>ClientInterviewFollowUp.values.byName(e as String)).toList(),interviewerReactions:(j['interviewerReactions'] as List).cast<String>(),accumulatedEvaluation:ClientInterviewEvaluation.fromJson(j['accumulatedEvaluation'] as Map<String,dynamic>),completed:j['completed'] as bool,deepDiveOccurred:j['deepDiveOccurred'] as bool? ?? false,mismatchFailure:j['mismatchFailure'] as bool? ?? false,deepDiveText:j['deepDiveText'] as String?,result:j['result']==null?null:ClientInterviewResult.values.byName(j['result'] as String),step:j['step']==null?SelectionStep.clientInterview:SelectionStep.fromJson(j['step'] as String));
}
