...
- **AI Replay Audit #3が発見した唯一のP1（Recruitment Interview Agency）を修正した。** 採用面談のinteractive Q&Aが開始される前から候補者カードに「評価」が表示され、実際のQ&Aでの回答が採用面談の合否・給与提示可否に一切影響していなかった問題——`PublicDemoApplicant.interviewScore`（候補者生成時に固定、面談開始前の値）のみが合否判定・給与提示ボタンの`>=60`ゲートを決めており、interactive Q&A（`RecruitmentInterviewEngine`が生成する実際の回答credibility）はcompanyImpression/candidateKnowledgeという表示専用メーターにしか反映されず、合否判定（プレイヤーの2択ボタン）はどちらの値も一切参照していなかった。
- **修正**: 既存authority（`PublicDemoApplicant.interviewScore`/`RecruitmentInterviewSession`）へ安全に統合する形で、新規`PublicDemoRecruitmentInterview.finalEvaluationScore`（純粋関数、面談セッション完了まで`null`）が実際に回答されたQ&Aのcredibilityをbaselineへ加減算し、カードの「評価」表示・給与提示ボタンの合否ゲート・HOME推奨アクションの3箇所すべてをこれへ差し替えた。「評価」表示自体も、面談が実際に完了し「採用候補として進める」が選択されるまで一切表示しないよう変更——面談前の結果先取り表示も同時に解消。同一candidateで質問セット次第で合否が実際に反転することをwidget testで直接証明済み。新規save schemaフィールド・新規RNGは追加していない。
- **PR #264のCodex Broad Review（Review ID 5191465045）がP1 2件を指摘し、両方本エントリで対応済み**:
  - **P1-1（Preserve eligibility for in-flight saved interviews）**: 上記修正をPR #264以前のsaveへ retroactiveに適用すると、旧authority（`interviewScore`単独、例: 61）では既に給与提示可能だった「面談完了・採用候補として進める選択済み・給与提示前」のcandidateが、新authority（実際のQ&A回答から導出、同じ例で54）では不合格に転落し、HOME/Sales両方から給与提示アクションが消え、面談は既に完了済みのため再実行もできないdead endになることが判明した。Fresh Auditの結論: 完了済みセッションのpersisted data（`applicantAnswers`/`completed`/`outcome`）はpre-fix/post-fixのどちらの版で決定されたかに関わらずbyte-identicalであり、既存データのみからは判別不能——純粋なdata-onlyでのgrandfather化は不可能と確認した。最小限の明示的migration/versioningとして、新規`PublicDemoApplicant.qaEvaluationApplies`（デフォルト`false`——本フィールドが存在しない旧saveも含む）を追加し、`PublicDemoAggregate.concludeInterviewSession`が「採用候補として進める」を確定させたその瞬間にのみ`true`をセットするようにした。`finalEvaluationScore`は`false`（旧saveの既決定）を元の`interviewScore`へgrandfather、`true`（このビルドが下した決定）は常に実際のQ&A評価を使う——単一authority内でのバージョン分岐であり、二重authorityではない。既存candidateの削除・面談リセット・ランダム再判定はいずれも行っていない。
  - **P1-2（Sync the completed task into the governing plan）**: 本エントリ自体がその対応——AI Replay Audit #3 / Recruitment Interview Agency P1 / PR #264 / backward compatibility fixを本文書へ記録した。上記「Relationship to existing documents」に`SES_FIRST-FUN-QUARTER_*_Result.md`系列の位置づけも新規追加。
- `flutter analyze`（全体）issue無し、`git diff --check`クリーン。P1-1専用focused test 8件（game層6件: pre-update save相当のeligibility維持・save/reload後の維持・新規interviewでのQ&A pass/fail反転・grandfather誤爆防止・duplicate retry・month boundary、UI層2件: HOME guided route・Sales direct routeでの給与提示可能維持）追加。`flutter test test/game/public_demo`（1028件）・`test/ui/public_demo`（フルスイート実行、結果は本エントリのResult Report参照）すべてPASS。Claude self-hardening review 1回実施。Codex Broad Reviewは本エントリでは再実行していない（指示通り）。
- 詳細・fresh audit・authority設計・backward compatibility戦略・テスト証跡は`docs/reports/SES_FIRST-FUN-QUARTER_Recruitment-Interview-Agency_P1_Result.md`を参照。PR: https://github.com/perusonao/smile_enjoy_story/pull/264 。
- **本エントリはCurrent execution order・Prioritized backlog tableの構成自体は変更しない**（直上の新規`Relationship to existing documents`の1行を除く）。HOME/Finance/Payroll/Matching式/salesCapacity semantics/save schemaの既存フィールドはいずれも無変更（`qaEvaluationApplies`は新規additive fieldのみ）。
- **First Fun Quarter（AI Replay Audit系列）はまだPASS判定ではない。** 残Gate: PR #264のmerge → Focused Human-like Replay（実プレイでのagency確認） → PASS判定、の順で未完了。次にこの系列へ着手する場合は、PR #264のmerge状態とReplay結果をまず確認すること。

### 2026-09-12（Issue #245 Finding #4 — FIRST-FUN-YEAR Parallel Sales Phase 1C「候補案件比較・受注選択UI」実装完了 / governing plan sync）

- **Phase 1a（PR #254）/Phase 1b（Issue #257）が積み上げた`offerCandidates`authorityの上に、プレイヤーが実際に触れる「候補案件を比較→受注案件を選択」UIを実装した。** 開始時`git fetch origin`で`origin/main`の`refs/heads/main`が指定base SHA（PR #258マージコミット）と完全一致していることを確認（drift無し）。
- **UI**: 社員タブの技術者アクションカード（`ec(i)`）に、候補案件が2件以上ある技術者には「候補案件を比較」、1件以下でも既に営業中（waiting/skillSheet以外・未受注）の技術者には軽量な「他の案件も提案する」CTAを追加。新しいフルスクリーン`PublicDemoOfferComparisonScreen`（縦カード形式、360/390px対応）で、候補ごとに案件名・種別・単価・取引先・パートナー/客先面談結果・candidate状態（提案中/パートナー面談 通過・不通過/客先面談 通過・不通過/受注/見送り、内部enum名は非表示）を表示し、「この案件を受注」（確認ダイアログ付き）・面談実施ボタン・「別の案件も提案する」ピッカーを提供。既存の単一案件受注導線（`ec(i)`の既存ボタン、`recordOrder`）は無変更。
- **Domain（必要最小限の拡張）**: `PublicDemoAggregate.recordOfferCandidateOrder`（新規、安全なcandidate-aware受注エントリポイント）は既存の`PublicDemoWorkflowState.recordOfferCandidateOrder`（Phase 1a由来のbuilding block）を拡張し、候補の受注+同一engineerの他のliveな候補のdeclineに加えて、`PublicDemoEngineerSales.syncOrderedFromCandidate`（新規）でengineer側coarse `stage`/`interviewRecord`を同期——これがないと`assignOrderedForMay`/`recoverLateYearAssignment`の既存`stage==ordered`判定を満たせず月次assignmentが作られない。`_freshOrderedAssignment`/`recoverLateYearAssignment`は、engineer自身の`genuineInterviewProjectId`が無い場合に`offerCandidates`側の受注済みcandidateから正しいprojectIdへフォールバックするよう拡張（既存の単一経路では両者が常に一致するため、既存挙動は無変更）。`proposeAdditionalOfferCandidate`/`canProposeAdditionalOfferCandidate`（新規）は、既に営業パイプライン中の技術者だけに2件目以降の提案を許可するUI向けゲート——`proposeMatch`/`withMatchingProposal`/`availableEngineersForMatching`は一切無変更（1件目の提案導線に触れない）。2件目以降の面談は既存のsafe building block（`evaluatePartnerInterviewForCandidate`/`evaluateClientInterviewForCandidate`、Phase 1aで実装済み・未配線だったもの）を使用——新しい面談エンジンは作っていない。
- **Self-hardeningで発見・修正したP0級の既存バグ2件**（いずれもPhase 1B自身が導入し、実save codecを通した回帰テストが一度も無かったため見過ごされていたもの）: (1) `PublicDemoSaveCodec._hasConsistentAuthorityFacts`のoffer candidate score floorが`clientScore>=60`固定だったが、Phase 1Bが接続した対話式Project Interview Gameplayの確率的合格（`ProjectInterviewEngine.roll`、`rate`が60未満でも合格し得る）はこの下限を満たさず、**通常プレイのみで到達可能な低スコア合格が実在するsaveを丸ごとreject**していた——`>=5`（実エンジンの実際のクランプ下限）へ修正。(2) 同チェックが`declined`状態のcandidateに残る正当な`interviewRecord`（受注時に他候補をdeclineする既存仕様が必然的に残す）を`clientInterviewPassed`/`ordered`以外だからと誤ってrejectしていた——本Findingの中心シナリオ（2候補が合格→1件受注→もう1件は自動見送り）そのものがこのバグを踏む。両方とも`>=5`かつ`declined`許容へ修正し、専用回帰テスト追加。
- `flutter analyze`（全体）issue無し、`flutter test test/game/public_demo`（1015件）・`test/ui/public_demo`（フルスイート実行・全件PASS）・Phase 1C専用focused tests（domain 19件+UI 6件）すべてPASS、`git diff --check`クリーン。詳細は`docs/reports/SES_FIRST-FUN-YEAR_Parallel-Sales_Phase1C_Result.md`。
- **本エントリはCurrent execution order・Prioritized backlog tableの構成自体は変更しない。** HOME/Finance/Payroll/Matching式/Interview outcome式/salesCapacity semantics/`ordered != assigned`はいずれも無変更。Parallel Sales / Offer Selection（Issue #245 Finding #4）はPhase 1a→1b→1cを通じて実装完了。次のFirst Fun Year優先タスクは`docs/decisions/SES_FIRST-FUN-YEAR_NEXT-PRIORITIES_2026-09-10.md`のexecution order（Aug–Feb Late-game FUN等）を参照。

### 2026-09-12（Issue #257 — FIRST-FUN-YEAR Parallel Sales Phase 1B「Production Cutover / Dual Authority Reconciliation」実装完了 / governing plan sync）

- **Issue #257を実装完了。** Phase 1a（直下のエントリ、PR #254）が additive のまま残していた `offerCandidates` を、実際のPublic Demo営業/面談/受注フローへ接続した。開始時`git fetch origin`で`origin/main`が指定base SHA `3dffd5c78b5c956dfc4f0804eb12887d17042eb1`（PR #254マージコミット）と完全一致していることを確認（drift無し）。
- **PR #254 Codex reviewが指摘した「key present ≠ fully migrated」問題を実際に解消した。** Phase 1aの一時的synthesis（`offerCandidates`キーが完全に不在の場合のみ発火する一回限りの移行）を、`PublicDemoWorkflowState._reconcileOfferCandidates`（キーの有無に関わらず`fromJson`で毎回実行される、真の・繰り返し可能なreconciliation）へ置き換えた。既存candidateがlegacy authorityより進んでいる場合は不用意に巻き戻さず、legacy authorityが進んでいる場合のみ安全にupgradeする（`declined`は例外——legacy authorityがどれだけ進んでも復活させない）。
- **既存本番呼び出し元をcandidate authorityへ接続した**: `proposeMatch`→`withMatchingProposal`（提案時にcandidateを同時作成/同期）、Partner Interview（`startPartnerInterview`/`concludePartnerInterview`、既存sales slot消費を維持）、Client Interview（`startProjectInterview`/`concludeProjectInterview`、0-slot維持）、`recordOrder`（受注対象candidateのみorderし、同一engineerの他のliveな候補candidateをatomicにdecline）。既存Interview engine/sessionは一切forkせず、同一の計算済み結果をcandidate側へも反映するだけ——二重評価・二重RNG消費なし。`PublicDemoEngineerSales.stage`は互換層として維持し、既存の`assignOrderedForMay`/`recoverLateYearAssignment`/`availableEngineersForMatching`は無変更。`ordered != assigned`は`recordOrder`のcandidate-aware化後も維持（`assignments`へは一切書き込まない）。
- **セルフレビューで2件の問題を発見・修正/棄却済み**: (1) 新規candidate作成が常に`proposed`から始まる実装は、legacy genericパスで既にpartner合格/不合格していたengineerに対して一時的な巻き戻れ状態（次回reloadまで）を作っていた——`withMatchingProposal`がengineerの現在のlegacy stageから直接candidateを生成するよう修正（reconciliationと同じ導出ロジックを共有）。(2) save codecの`_hasConsistentAuthorityFacts`に追加検討した「ordered candidateのprojectIdは現在のmatchingProposal/assignmentと一致必須」という厳格チェックは、`endAssignment`→再営業→別案件で再受注という正当な複数サイクル履歴（`offerCandidates`は履歴を削除しない）を誤って拒否するリスクがあったため実装前に発見し、「ordered candidateのengineer自身もgenuinely orderedであること」のみを要求する緩い版へ修正した——過剰な検証で正当なsaveを壊すリスクを避けた。
- `flutter analyze`（プロジェクト全体）No issues、`flutter test test/game/public_demo`（980件）・`flutter test test/ui/public_demo`（741件、UI無変更のため既存件数のままgreen）・`git diff --check`いずれもgreen。詳細・authority before/after・reconciliation戦略・production caller cutover map・legacy save matrixは`docs/reports/SES_FIRST-FUN-YEAR_Parallel-Sales_Phase1B_Result.md`を参照。
- **PR #258レビューfollow-up（同日中に対応、mainへの初回マージ前）**: Codex Broad Reviewはusage limit到達により実質未実施だったため、別セッションのClaude Independent Broad Reviewを本タスクの「一回だけのBroad Review」の代替として位置づけた（詳細は`docs/reports/SES_PR-258_Parallel-Sales-Phase1B_Claude-Broad-Review.md`）。このレビューが1件のP1（`withMatchingProposal`が、engineerの2件目以降のcandidate作成時に無関係な別projectの既に獲得済みinterview stage/scoreを誤って継承し、2件目のproject自身のPartner Interviewを実質スキップさせてしまうbug——通常のproposeMatch→面談→proposeMatch(2件目)という普通のプレイで再現する、fake saveではない実バグ）を発見・修正済み（commit `e7996e2`）。加えて、Issue #257本文がこのセッションの初回調査後に編集され「PR #256 Review Carry-over」節（`projectInterviewSessions`の`(employeeId, projectId)`複合キー化を必須と明記）が追加されていたことが判明したため、レビューがP2として指摘したこの項目を本セッションで実装まで完了させた（単なる既知の制限としての記録に留めなかった）——`startProjectInterviewSession`のreplace範囲を同一`(employeeId, projectId)`ペアのみに限定し、`projectInterviewSessionFor`を複合キー化。既存の`PublicDemoAggregate.projectInterviewSessionFor(engineerId)`の公開シグネチャは無変更のため、UI/既存テストの呼び出し箇所は一切変更不要だった。全修正後、`flutter test test/game/public_demo`（980件）・`test/ui/public_demo`（741件）・`flutter analyze`・`git diff --check`いずれも再green確認済み。
- **本エントリはCurrent execution order・Prioritized backlog tableの構成自体は変更しない。** Phase 1c（候補案件比較UI）は本セッションでは未着手のまま——次にParallel Sales/Offer Selectionへ着手する場合は、このエントリが指すResult Reportの「Known limitations / unresolved Phase 1C」から開始する。HOME/Finance/Payroll/Matching式/Interview outcome式/salesCapacity semanticsはいずれも無変更。
