...
// same assignment through the fiscal year's close in March, with no further
// monthly renewal decision — see SES_12MONTH-3_P1_Fixes_Result.md. This test
// exercises that *specific* contract end-to-end through the real widget, and
// proves Revenue's engineersAssigned count and Growth's assignment-source
// identity set stay the single source of truth for each other every month
// (the P1-1 SSOT bug: before this fix, Growth kept crediting "assignment"
// growth to an engineer Revenue had already stopped counting as assigned).
//
...
void main() {
  testWidgets('a July-accepted assignment carries forward through March, with '
      'Revenue and Growth agreeing on the same assigned headcount every '
      'ordinary month (12MONTH-3-FIX1 P1-1 contract)', (tester) async {
    ...
    expect(grewByAssignment(state, 'eng-01'), isTrue, reason: 'E (September)');
    expect(
      state.latestGrowthResults
          .where((r) => r.source == PublicDemoGrowthSource.assignment)
          .length,
    ...
    // Close the remaining ordinary months through March.
    ...
    // G. Fiscal year completes; March's own revenue stays pending, not
    // swept into cash (unchanged 12MONTH-3 contract).
    state = currentState(tester);
    expect(state.month, 15);
    expect(state.fiscalYearCompleted, isTrue, reason: 'G');
  });
}
